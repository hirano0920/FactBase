...
export interface GenerateArticleParams {
  ...
  /**
   * ABEMA Prime討論回をGemini動画理解APIで解析した結果（discover-abema.ts経由の候補のみ）。
   * Writerは forBullets / againstBullets に書かれた発言者名・主張をそのまま帰属引用として使ってよい
   * （動画を実際に視聴した上での抽出のため、報道記事の抜粋と同格の一次資料として扱う）。
   */
  abemaPrime?: {
    videoTitle: string;
    videoUrl: string;
    /** チャンネル名。未設定は"ABEMA Prime"（伝説級枠のReHacQ/NewsPicks等でも同じ構造） */
    channel?: string;
    lead: string;
    axis: string | null;
  ...
  /**
   * 伝説級（過去のバズり動画）由来の争点で、動画公開後に関連法令が実際に成立した場合。
   * 「当時は賛否が割れていたが、その後実際に成立した」というギャップをWriterに明示し、
   * 「今も賛成/反対を問う」設問ではなく「振り返ってどう思うか」の回顧的設問に切り替えさせる。
   */
  resolvedSinceVideo?: {
    lawTitle: string;
    promulgationDate: string;
...
出典: ${dietVote.sourceUrl}`
  ...
  const abemaPrimeBlock = abemaPrime
    ? `\n\n# ★ ABEMA Prime討論回（動画を実際に視聴した上での抽出。発言者名は一字一句そのまま）★
番組: ${abemaPrime.videoTitle}
要約: ${abemaPrime.lead}${abemaPrime.axis ? `\n対立軸: ${abemaPrime.axis}` : ""}
${abemaPrime.forBullets.length > 0 ? `${abemaPrime.forLabel ?? "賛成側"}:\n${abemaPrime.forBullets.map((b) => `- ${b}`).join("\n")}` : ""}
${abemaPrime.againstBullets.length > 0 ? `${abemaPrime.againstLabel ?? "反対側"}:\n${abemaPrime.againstBullets.map((b) => `- ${b}`).join("\n")}` : ""}
${abemaPrime.keyPoints.length > 0 ? `ポイント:\n${abemaPrime.keyPoints.map((k) => `- ${k}`).join("\n")}` : ""}
出典: ${abemaPrime.videoUrl}`
    : "";

  const resolvedSinceVideoBlock = resolvedSinceVideo
    ? `\n\n# ★★★ 重要: この動画の論点は動画公開後に決着済み ★★★
動画公開日: ${new Date(resolvedSinceVideo.videoPublishedAt).toLocaleDateString("ja-JP")}
その後成立: ${resolvedSinceVideo.lawTitle}（成立日: ${resolvedSinceVideo.promulgationDate}）
この動画が議論していた「賛成すべきか反対すべきか」は、動画公開後に実際に決着している。
「今も賛否が割れている」かのような設問・書き方は事実と矛盾するため厳禁。
必ず以下の構成で書くこと:
1. 冒頭で「当時この動画ではこう議論されていたが、その後${resolvedSinceVideo.lawTitle}として実際に成立した」という経緯を明示する
2. 賛否セクションは「動画公開当時の賛成派/反対派の主張」として過去形で扱う
3. 設問・見出しは「振り返ってどう思うか」「この判断は妥当だったか」という回顧的な問いにする（「賛成ですか？」という現在進行形の問いにしない）`
    : "";
...
# この争点の書き方（厳守）
${typeHint}
${excerptBlock}${claimDiffHint}${reportExcerptBlock}${internationalReportExcerptBlock}${pollingExcerptBlock}${dietBlock}${datedExcerptBlock}${backgroundBlock}${lawsBlock}${estatBlock}${estatFiguresBlock}${dietVoteBlock}${abemaPrimeBlock}${resolvedSinceVideoBlock}${previousBlock}${revisionBlock}
...
export function buildSourceTextIndex(
  params: Partial<Pick<GenerateArticleParams, "sources" | "abemaPrime">> &
    Pick<
  ...
  if (params.abemaPrime) {
    const ap = params.abemaPrime;
    const text = [ap.lead, ap.axis, ...ap.forBullets, ...ap.againstBullets, ...ap.keyPoints]
      .filter(Boolean)
      .join("\n");
    index.set(ap.videoUrl, text);
  }
