...
export function debateTypeBulletsSpec(debateType: DebateType, isReported: boolean): string {
  ...
  switch (debateType) {
    ...
    case "norm_flare":
      // 旧「争点の軸: …（何の規範・行為が問題になっているか具体的に）」は「〜を巡る規範的な対立」
      // のような対立の説明に終始し、実際に何が言われた/行われたか（発言の要旨や言葉そのもの）が
      // 抜け落ちる出力を誘発していた（読者が争点そのものを把握できず投票できない実例あり）。
      // 他の型と同じ「いま分かっていること」形式にし、発言・行為の中身を明示させる。
      return `["いま分かっていること: …（誰が・どこで・何を発言/行為したか、発言の要旨や言葉そのものを含めて具体的に。その後どう対応されたか）", "擁護側: 芯。根拠。", "批判側: 芯。根拠。"]。${density}`;
    case "indicator":
