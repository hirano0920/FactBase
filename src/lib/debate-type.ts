...
「いま何が論点か」必須スロット（空欄・抽象文禁止）:
  ...
  ② [時期・場面]
  ③ [否定・反応と対立軸]
両側セクションは事実の再掲禁止。各側の主張・反論・論点だけ。各項目に発言・声明・報道の帰属。
きれいに対称でなくてもよい（実争点は非対称が多い）。ただし片側だけ抽象スローガンは禁止。`;
    case "policy":
...
各<li>に議員名・政党・媒体の帰属。暮らし・税金・権利への影響を対立軸に含める。`;
    case "org_response":
...
両側とも公式コメント・報道の具体点から。教科書的な「信頼回復が必要」だけの埋め禁止。`;
    case "norm_flare":
...
日本への波及（貿易・エネルギー・為替・安保）があれば対立軸に落とす。片側だけの抽象スローガン禁止。`;
  }
}
...
export function debateTypeTitleHint(debateType: DebateType): string {
  switch (debateType) {
    ...
    case "org_response":
      return "フックは実際の消費者・利用者・従業員への具体的な影響（返金・補償・利用停止・処分内容等）を事実で示す。「自分が〇〇なら」という一般化された仮定フックは使わない（空虚になるため禁止）。対応が二転三転した話なら型②、対応の是非そのものが争点なら型①か⑧が合う。企業名・対応内容を具体的に。";
    case "norm_flare":
...
export function debateTypeBulletsSpec(debateType: DebateType, isReported: boolean): string {
  ...
  switch (debateType) {
    case "declaration":
      return `["報道の内容: …（何をした／何を言ったか。事実のみ簡潔に。否定は書かない）", "A側（当事者名）: 芯の一文。根拠の一文。", "B側（当事者名）: 芯の一文。根拠の一文。"]。${density}`;
    case "org_response":
      return `["いま分かっていること: …（何が起き、組織が何をしたか）", "対応を支持する側: 芯。根拠。", "問題だとする側: 芯。根拠。"]。${density}`;
    case "norm_flare":
      return `["いま分かっていること: …（誰が・どこで・何を発言/行為したか、発言の要旨を含めて具体的に。その後どう対応されたか）", "擁護側: 芯。根拠。", "批判側: 芯。根拠。"]。${density}`;
    case "indicator":
