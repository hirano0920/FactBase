...
export async function factCheck(commentBody: string, chunks: FcChunk[]): Promise<FcResult> {
  ...
  const parsed = JSON.parse(raw) as { v?: string; l?: string; r?: string; s?: string[] };
...
export async function judgeModeration(
  ...
  const raw = res.choices[0]?.message?.content ?? "{}";
  const parsed = JSON.parse(raw) as Partial<ModerationJudgement>;
...
export async function judgeIssueQuality(
  ...
  const parsed = JSON.parse(res.choices[0]?.message?.content ?? "{}") as Partial<IssueQualityJudgement>;
  return {
 ...
 */
const RADAR_CLASSIFY_PROMPT = `あなたはニュース編集デスクです。与えられた見出しリストを「同じ出来事」ごとにクラスタリングし、各クラスタを分類してください。
...
# category
politics / economy / law / finance / rights / international / education のいずれか

# risk_flags（該当するものすべて。慎重に倒す）
