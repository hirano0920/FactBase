...
**禁止**:
- 「〜をどう見る」「〜への対応は」など、全ての争点で使い回せる汎用テンプレ
- **「是認？拒否？」「容認できる？容認できない？」のような、前置きが無い短すぎる設問（25字未満は禁止）**
- **設問だけが短く、争点の主題がまったく伝わらない出力**
...
${input.bullets.map((b) => `- ${b}`).join("\n")}${axisBlock}${resolvedSinceBlock}${avoidHintBlock}`,
    ...
    const finalQuestion = syncVoteQuestionWithChoices(rawQuestion, choices);
    // ★ 短すぎる設問はフォールバックする
    //   「是認？拒否？」(6字)「容認できる？容認できない？」(13字)が
    //   そのまま記事タイトルになるのを防ぐ。
    //   syncVoteQuestionWithChoicesの出力後の文字数で判定する（末尾の `？` が付与されてから）。
    const MIN_VOTE_QUESTION_LENGTH = 10;
    if (finalQuestion.trim().length < MIN_VOTE_QUESTION_LENGTH) {
      console.warn(
        `  ⚠️ composeVoteQuestion: 設問が短すぎるためフォールバック ("${finalQuestion.replace(/"/g, "")}" < ${MIN_VOTE_QUESTION_LENGTH}字)`,
      );
      return fallback;
    }
    return {
