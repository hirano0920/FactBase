...
# タイトルの黄金比: 「数字・規模」> 「意外性・対立」> 「切迫・危機・暴露」> 「個別具体フック」
どのトピックにも当てはまるわけではないが、以下の4要素のうち使えるものを**優先順に**拾え:

## 0. ★確定した対立軸が与えられた場合: それを軸にしろ
与えられた「側A/側B」の対立をタイトルに直接使え。
- 側A「自衛権の範囲内」 vs 側B「国際法違反」 → 「自衛権か国際法違反か、イラン攻撃の正当性を問う」
- 側A「教育ビザで雇用継続」 vs 側B「制裁違反で禁止」 → 「国連制裁の抜け穴？北朝鮮労働者雇用の賛否」
...
## 2.5 切迫・危機・暴露（争点に緊迫感があればこれ）
終わりの見えない事態、エスカレートする緊張、制度の抜け穴。
- 「止まらないイラン報復、いつまで続く？日本経済への影響は？」
- 「国連ルールの抜け穴で北朝鮮労働者を大量受け入れか？」
- 「エスカレートする中東、日本は巻き込まれる？」
- 「"NVIDIA来日、日本企業は半導体競争に乗り遅れる？」
...
const VOTE_QUESTION_PROMPT = `あなたは${SITE.name}の編集デスクです。書き終えた記事の内容から、**記事タイトル兼投票設問**と選択肢を作ります。

# 絶対原則（重要度順）
1. **読者が「なに！？」「それ知りたい！」と止まる** — 記事タイトルとして最初に目に入る
2. **選択肢は直感的で、状況に合っていること**（軍事攻撃なのに「停戦優先？圧力継続？」はおかしい）
3. **「支持？不支持？」にデフォルトするな** — 必ず争点固有の対立軸を選べ
4. **緊急感・切迫感・意外性を盛り込め** — 「ただの賛否」では誰もクリックしない
...
| 状況 | ❌ ダメな例 | なぜダメ？ | ✅ 正しい例 |
|------|-----------|-----------|-----------|
| 米軍のイラン空爆継続 | 「攻撃継続は正当？不当？」 | ドライすぎてニュース速報と変わらない。読者の感情に触れない | **「止まらないイラン報復、いつまで？日本経済への波及は？」** choices:懸念/過小評価 |
| 終わりの見えない軍事衝突 | 「停戦を支持？不支持？」 | 選択肢が状況にあってない。当事者でもないのに「支持」は不自然 | **「エスカレートする中東、日本は巻き込まれる？距離を置くべき？」** choices:巻き込まれる/距離を置く |
| 制度の抜け穴スキャンダル | 「ルール改正は必要？不要？」 | 漠然としすぎ。「まさか」感がゼロ | **「国連ルールの抜け穴で北朝鮮労働者を大量受け入れか？」** choices:規制強化/現状維持 |
| 食料品減税（誰も反対しない政策） | 「支持？不支持？」 | 反対する人がほぼいない政策に賛否はおかしい | **「2年で十分？不十分？」** / **「食料品だけ？他も減税すべき？」** |
| 値上げの是非 | 「支持？不支持？」 | 値上げに「支持」は不自然 | **「妥当？不当？」** / **「賛成？反対？」** |
| **lockedAxisあり** | 無視して「支持？不支持？」 | 現実の対立軸を無視 | **sideA→for、sideB→againstに圧縮** |

# question（55字以内・必ず日本語）
**必ずしも「XはY？Z？」形式でなくてよい。** 以下のパターンを使い分けよ:

1. **切迫/危機パターン**: 「止まらない[事態]、[影響]は？いつまで？」 ← 軍事・災害・経済危機
   - 例:「続くイラン報復、日本経済への波及は？」
   - 例:「エスカレートする中東、いつまで続く？日本は巻き込まれる？」

2. **驚き/暴露パターン**: 「[意外な事実]が[起きている]？...」 ← スキャンダル・制度の抜け穴
   - 例:「国連ルールの抜け穴で北朝鮮労働者を大量受け入れか？」
   - 例:「NVIDIA来日、日本企業への影響は？半導体競争に乗り遅れる？」

3. **対立軸パターン**: 「[主題]は[choice1]？[choice2]？」 ← 政策論争・価値観対立（従来型）
   - 例:「加熱式たばこ40円値上げは妥当？不当？」

**禁止**: 「〜をどう見る」「〜への対応は」など、全ての争点で使い回せる汎用テンプレ

# choices（for/against/undecided）
- **forとagainstはquestionの末尾の対義語ペアと完全一致させよ**
- 各2〜4字推奨（最大でも5字） — 短く切れ
- 短い対義語ペアの例: 妥当/不当、必要/不要、是認/拒否、正当/不当、是/非、賛成/反対、支持/不支持、容認/拒否、肯定/否定、推進/慎重、拡大/縮小、優先/後回し、懸念/過小評価、関与/距離、規制/緩和
- **lockedAxis.sideA/sideBが与えられた場合**: sideAをfor(肯定側)に、sideBをagainst(否定側)に圧縮した形を最優先で検討する
- undecided: 「どちらとも言えない」「まだ判断できない」「判断保留」
- questionがパターン1・2の場合は、末尾の対義語ペアにこだわらず、争点の本質を捉えた短い選択肢にしてもよい
 ...
 */
const DEBATE_LEGITIMACY_PROMPT = `あなたは${SITE.name}編集部の審査員です。あるニューストピックを「両論記事として扱うべきか」を判定します。

判定は**以下4ステップの判定ツリー**に沿って機械的に行ってください。感覚ではなくルールで判断します。

# トピックの種類をまず認識する

A) **政治/法律/政策** → 賛成vs反対、支持vs不支持の古典的な二元対立
B) **経済ショック/企業ニュース/株式相場** → 「事象をどう解釈するか」の解釈対立
C) **外交/安全保障** → 対応の是非やリスク認識の差
D) **スキャンダル/不祥事** → 擁護vs批判、責任の所在
E) **社会現象/技術革新** → メリットvsデメリット

経済ショック（B）では伝統的な「賛成/反対」が意味をなさないことがあるが、
解釈や評価が分かれるなら「両論あり」と判定してよい。
...
## Step 2: 与えられた情報の中に、対立する立場が見えるか
渡された「確定論点」「立場A/B」「媒体間の食い違い」「抜粋」を総合的に判断。
...
- **「確定論点」が明示されている（軸ロック済み＝現実の対立軸が特定されている）**
- **「媒体間の食い違い」がある（複数の報道機関が異なる角度から報じている）**
- **経済ショック/相場変動の場合**: 複数媒体が異なる解釈・分析を報じている（楽観vs悲観、一時的vs構造的）
...
export async function assessDebateLegitimacy(params: {
  ...
  category?: string;
  /**
   * 軸ロック情報（lockedAxis.axis/sideA/sideB）。これが既にあれば
   * 「現実の対立軸が確定している」証拠として Step2 の判断材料にする。
   */
  lockedAxis?: { axis: string; sideA: string; sideB: string };
  /**
   * Yahoo!投票の実測分断スコア（0〜1）。0.2以上（≒上位60%対40%の開き）なら
   * 実際の読者が拮抗している証拠としてLLM呼び出しをスキップできる。
   */
  externalPollDivision?: number;
  /**
   * Yahoo!コメントの摩擦度（0〜1）。0.3以上ならコメント欄で実際に論争が起きている証拠。
   */
  commentFrictionScore?: number;
  /**
   * 複数媒体間の主張の食い違い。1件以上あれば現実に論点が存在する証拠。
   */
  ...
  // ★ 実測データによるショートカット:
  // 実際の投票で拮抗・コメントで論争・媒体間で食い違い → すでに両論性は確定している。
  // LLM呼び出し不要で通過させる（API節約＋誤診断防止）。
  const hasStrongDebateSignal =
    (typeof params.externalPollDivision === "number" && params.externalPollDivision >= 0.2) ||
    (typeof params.commentFrictionScore === "number" && params.commentFrictionScore >= 0.3) ||
    (params.claimDiffConflicts && params.claimDiffConflicts.length >= 1) ||
    // lockedAxisがvoteQuestion由来（pollから確定）なら信頼性が高い
    (params.lockedAxis !== undefined && typeof params.externalPollDivision === "number");

  if (hasStrongDebateSignal) {
    return {
      legitimate: true,
      problemType: "ok",
      reason: "実測データ（投票/コメント/媒体食い違い）で両論性を確認",
      suggestedFrames: [],
      predictedDivisionScore: typeof params.externalPollDivision === "number"
        ? params.externalPollDivision
        : typeof params.commentFrictionScore === "number"
  ...
  try {
    ...
    const userContent = [
      ...
      // 軸ロック情報がある場合は追加（「現実に対立軸が確定している」証拠）
      params.lockedAxis ? `【確定論点】${params.lockedAxis.axis}\n【立場A】${params.lockedAxis.sideA}\n【立場B】${params.lockedAxis.sideB}` : "",
      // claimDiffの食い違いがあれば追加（「複数媒体が異なる主張を報じている」証拠）
      params.claimDiffConflicts && params.claimDiffConflicts.length > 0
        ? `【媒体間の食い違い】\n${params.claimDiffConflicts.slice(0, 3).map((c) => `  - ${c}`).join("\n")}`
        : "",
