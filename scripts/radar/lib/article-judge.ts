...
6. clarity（構造・わかりやすさ）: 見出し構成・箇条書きが分かりやすいか。
...
/**
 * 本番公開ゲート用に見る軸。ニュースサイト向け指標（depth/clarity/titleHook等）は
 * 「読み物として優れているか」で議論場の要件ではないため、公開可否には使わない。
 * 議論場（TwoSides）が最低限満たすべき「両論バランス」と「中立性」だけをゲートにする。
 */
export const QUALITY_GATE_AXES = ["bothSidesQuality", "neutrality"] as const satisfies readonly (keyof ArticleJudgeScore)[];
