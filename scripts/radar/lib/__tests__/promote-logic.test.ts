...
describe("selectTopicsForPromotion", () => {
  ...
  it("コメントランキング救済: minBuzz未満でも Buzz'≥0.4 かつ Heat' があれば通す", () => {
    const candidates: PromotionCandidate[] = [
      ...
      // ただしtweetCountが100未満の場合のみ（≥100はBuzz低でも通過可にする逃げ道）
      candidate("too-weak-buzz", {
        category: "society",
        evidence: ev({
          buzzScore: 1,
          tweetCount: 80, // tweetCount < 100 → Buzz低でも通せない
          news: sufficientNews,
          buzzSources: ["yahoo_comment_ranking"],
          debateType: "norm_flare",
        }),
  ...
  it("debatable=falseはNews候補としてeligibleに残す", () => {
    const candidates: PromotionCandidate[] = [
      candidate("news", {
        title: "キオクシア株急落",
        category: "economy",
        evidence: ev({
          ...
          debateType: null,
          commentCount: 200,
          commentFrictionScore: 0.2,
        }),
      }),
    ];
    expect(selectTopicsForPromotion(candidates, 2, 3).map((c) => c.id)).toEqual(["news"]);
  });
  ...
  it("debatable=falseのNews候補もDebate候補もeligibleに残る", () => {
    const candidates: PromotionCandidate[] = [
      {
        id: "news",
        title: "キオクシア株急落",
    ...
    const selected = selectTopicsForPromotion(candidates, 2, 3);
    expect(selected.map((c) => c.id).sort()).toEqual(["debate", "news"]);
  });

  it("debatable未指定（旧データ）はtrue扱いで通し、falseはNewsとして通す", () => {
    const candidates: PromotionCandidate[] = [
      {
    ...
    const selected = selectTopicsForPromotion(candidates, 2, 3);
    expect(selected.map((c) => c.id).sort()).toEqual(["false-debatable", "undefined-debatable"]);
  });
...
describe("findDuplicateActiveIssue", () => {
  const activeIssues: ActiveIssueForDedup[] = [
    { id: "issue-1", title: "国旗損壊罪の新設をどう見る？", keywords: ["国旗損壊罪"], createdAt: new Date() },
    { id: "issue-2", title: "日銀の利上げ判断をどう見る？", keywords: ["日銀利上げ"], createdAt: new Date() },
  ];
